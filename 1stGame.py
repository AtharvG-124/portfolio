...
WINNER_FONT = pygame.font.SysFont('Nova Square', 100)
...
def window_features(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    WIN.blit(SPACE, (0, 0))
...
def yellow_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - SPEED > 0:  # left
        yellow.x -= SPEED
    if keys_pressed[pygame.K_d] and yellow.x - SPEED + yellow.width < BORDER.x + 3:  # right
        yellow.x += SPEED
...
def red_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - SPEED > BORDER.x + BORDER.width:  # left
        red.x -= SPEED
    if keys_pressed[pygame.K_RIGHT] and red.x + SPEED + red.width < WIDTH + 15:  # right
        red.x += SPEED
...
def check_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
...
def main():
    red = pygame.Rect(675, 225, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
    yellow = pygame.Rect(225, 225, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
    ...
    while run:
        ...
        check_bullets(yellow_bullets, red_bullets, yellow, red)

        window_features(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health)
